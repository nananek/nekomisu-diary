...
MiAuth で認証した Misskey アカウントが連携済みなら、その場でログイン
成立します。パスキーでのログインと同様、MiAuth 認証自体を十分に強い
資格情報とみなすため、日記側で TOTP / WebAuthn を設定していても
2段階認証はスキップされます（2段階認証は Misskey 側で設定してください）。
連携されていない Misskey アカウントではログインできません。
